...
var prepare = (color, mode) => color === void 0 ? void 0 : typeof color !== "object" ? parse_default(color) : color.mode !== void 0 ? color : mode ? __spreadProps(__spreadValues({}, color), {
  mode
}) : void 0;
...
var parseTransparent = (c2) => c2 === "transparent" ? {
  mode: "rgb",
  r: 0,
  g: 0,
  b: 0,
...
var differenceEuclidean = (mode = "rgb", weights = [1, 1, 1, 0]) => {
  ...
  return (std, smp) => {
    ...
    return Math.sqrt(channels.reduce((sum, k4, idx) => {
...
var differenceCmc = (l = 1, c2 = 1) => {
  ...
  return (std, smp) => {
    ...
    let Sh = Sc * (F * T + 1 - F);
    return Math.sqrt(dL2 / Math.pow(l * Sl, 2) + dC2 / Math.pow(c2 * Sc, 2) + dH2 / Math.pow(Sh, 2));
  };
};
var differenceHyab = () => {
  let lab2 = converter_default("lab65");
...
function convertRgbToHsi({
  ...
  if (M3 - m !== 0) res.h = (M3 === r2 ? (g - b) / (M3 - m) + (g < b) * 6 : M3 === g ? (b - r2) / (M3 - m) + 2 : (r2 - g) / (M3 - m) + 4) * 60;
  if (alpha !== void 0) res.alpha = alpha;
  return res;
}
...
var convertItpToXyz65 = ({
  ...
  if (t === void 0) t = 0;
  if (p4 === void 0) p4 = 0;
  const l = transferPqDecode(i + 0.008609037037932761 * t + 0.11102962500302593 * p4);
  const m = transferPqDecode(i - 0.00860903703793275 * t - 0.11102962500302599 * p4);
  const s = transferPqDecode(i + 0.5600313357106791 * t - 0.32062717498731885 * p4);
  const res = {
    mode: "xyz65",
    x: toRel(2.070152218389422 * l - 1.3263473389671556 * m + 0.2066510476294051 * s),
...
var convertXyz65ToItp = ({
  ...
  const absY = toAbs(y);
  const absZ = toAbs(z);
  const l = transferPqEncode(0.3592832590121217 * absX + 0.6976051147779502 * absY - 0.0358915932320289 * absZ);
  const m = transferPqEncode(-0.1920808463704995 * absX + 1.1004767970374323 * absY + 0.0753748658519118 * absZ);
  const s = transferPqEncode(0.0070797844607477 * absX + 0.0748396662186366 * absY + 0.8433265453898765 * absZ);
  const i = 0.5 * l + 0.5 * m;
...
var convertLrgbToOklab = ({
  ...
  let M3 = Math.cbrt(0.2119034981999999 * r2 + 0.6806995450999999 * g + 0.1073969566 * b);
...
var convertOklabToLrgb = ({
  ...
  let M3 = Math.pow(l * 1.0000000088817609 - 0.10556134232365635 * a - 0.06385417477170591 * b, 3);
...
var convertRgbToXyb = (color) => {
  const {
    ...
    b,
    alpha
  } = convertRgbToLrgb_default(color);
...
function clampChroma(color, mode = "lch", rgbGamut = "rgb") {
  ...
  let clamped = __spreadProps(__spreadValues({}, color), {
    c: 0
  });
  ...
  return conv(inDestinationGamut(clamped) ? clamped : __spreadProps(__spreadValues({}, clamped), {
    c: _last_good_c
  }));
}
function toGamut(dest = "rgb", mode = "oklch", delta = differenceEuclidean("oklch"), jnd = 0.02) {
  ...
  return (color) => {
    ...
    if (candidate.l >= ranges.l[1]) {
      const res = __spreadProps(__spreadValues({}, destMode.white), {
        mode: dest
      });
    ...
    if (candidate.l <= ranges.l[0]) {
      const res = __spreadProps(__spreadValues({}, destMode.black), {
        mode: dest
      });
      if (color.alpha !== void 0) {
        res.alpha = color.alpha;
...
var matrixSepia = (amount) => {
  ...
  return [0.393 + 0.607 * a, 0.769 - 0.769 * a, 0.189 - 0.189 * a, 0, 0.349 - 0.349 * a, 0.686 + 0.314 * a, 0.168 - 0.168 * a, 0, 0.272 - 0.272 * a, 0.534 - 0.534 * a, 0.131 + 0.869 * a, 0, 0, 0, 0, 1];
};
var matrixSaturate = (sat) => {
  let s = minzero(sat);
  return [0.213 + 0.787 * s, 0.715 - 0.715 * s, 0.072 - 0.072 * s, 0, 0.213 - 0.213 * s, 0.715 + 0.285 * s, 0.072 - 0.072 * s, 0, 0.213 - 0.213 * s, 0.715 - 0.715 * s, 0.072 + 0.928 * s, 0, 0, 0, 0, 1];
};
var matrixGrayscale = (amount) => {
  let a = 1 - clamp2(amount);
  return [0.2126 + 0.7874 * a, 0.7152 - 0.7152 * a, 0.0722 - 0.0722 * a, 0, 0.2126 - 0.2126 * a, 0.7152 + 0.2848 * a, 0.0722 - 0.0722 * a, 0, 0.2126 - 0.2126 * a, 0.7152 - 0.7152 * a, 0.0722 + 0.9278 * a, 0, 0, 0, 0, 1];
};
var matrixHueRotate = (degrees) => {
  let rad = Math.PI * degrees / 180;
  let c2 = Math.cos(rad);
  let s = Math.sin(rad);
  return [0.213 + c2 * 0.787 - s * 0.213, 0.715 - c2 * 0.715 - s * 0.715, 0.072 - c2 * 0.072 + s * 0.928, 0, 0.213 - c2 * 0.213 + s * 0.143, 0.715 + c2 * 0.285 + s * 0.14, 0.072 - c2 * 0.072 - s * 0.283, 0, 0.213 - c2 * 0.213 - s * 0.787, 0.715 - c2 * 0.715 + s * 0.715, 0.072 + c2 * 0.928 + s * 0.072, 0, 0, 0, 0, 1];
};
